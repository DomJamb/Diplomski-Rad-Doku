...
Ugrađivanje $\bm{v}^i$ jednoznačno je određeno primjerom $\bm{x}^i$, tako da vrijedi:
...
Koristeći Bayesovu formulu, vjerojatnost $p_{\bm{\theta}, \bm{\phi}, \bm{\pi}}(l^i | \bm{x}^i)$ sada možemo definirati kao mješavinu distribucija vMF:
...
Parametri definiranog modela tada su: skup razrednih prototipova tj.\ skup čistih prototipova $\bm{\phi} = \{\bm{\mu}_1,\,...\,,\bm{\mu}_K\}$, koeficijenti miješanja $\bm{\pi}$ te parametri $\bm{\theta}$ ekstraktora značajki $g_{\bm{\theta}}$.\ 
...
Skup svih parametara modela označit ćemo s $\bm{\Omega} = \bm{\theta} \cup \bm{\pi} \cup \bm{\phi} \cup \bm{\psi}$.\ Nakon što smo parametrizirali obje potrebne distribucije, vrijeme je za definirati algoritam učenja.\
